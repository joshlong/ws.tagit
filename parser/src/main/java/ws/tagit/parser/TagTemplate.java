package ws.tagit.parser;

import org.antlr.runtime.ANTLRReaderStream;
import org.antlr.runtime.CommonTokenStream;
import ws.tagit.parser.impl.TagLexer;
import ws.tagit.parser.impl.TagParser;
...
public class TagTemplate {
    ...
    private String normalizedTagString(String i) {
        String input = i == null ? "" : i.trim();

        List<String> strings = Stream.of("'", "\"")
                .filter(eps -> input.startsWith(eps) && input.endsWith(eps))
                .filter(e -> e.length() > 1)
                .map(inputString -> inputString.substring(1).substring(0, inputString.length() - 1))
    ...
    private List<String> normalizeListOfTags(List<String> input) {
        return input.stream().map(this::normalizedTagString).collect(Collectors.toList());
    }

    public Set<Tag> tags(String tags) {
        try {
            return normalizeListOfTags(extractTagsFromString(tags))
                    .stream()
                    .map(t -> new Tag(cleanTag(t), t))
    ...
    private String stringOrNull(String i) {
        return (i + "").trim().toLowerCase().equals("") ? null : i;
    }

    private String cleanTag(String input) {
        return Optional.of(stringOrNull(input))
                .map(body -> {
                    Stemmer stemmer = new Stemmer();
                    body.chars().mapToObj(i -> (char) i).filter(Character::isLetterOrDigit).collect(Collectors.toList()).forEach(stemmer::add);
                    stemmer.stem();
    ...
    public static void main(String[] args) {
        TagTemplate theTagParser = new TagTemplate();
        Stream<Tag> tagStream = theTagParser.tags("cow #cow 'cows and dogs' \"cows and dogs\" dogs").stream();
        tagStream.forEach(System.out::println);
    }
 ...
 */
    ...
    private final static class Stemmer {
        private char[] b;
